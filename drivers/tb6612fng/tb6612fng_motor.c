...
static int _drive_continous(const struct device *dev, struct motor_power_config power_config)
{
    ...
    {
        if (power_config.direction == MOTOR_DIRECTION_CLOCKWISE)
        { // CW
            gpio_pin_set_dt(&conf->gpio1, 1);
            gpio_pin_set_dt(&conf->gpio2, 0);
        }
        else if (power_config.direction == MOTOR_DIRECTION_COUNTER_CLOCKWISE)
        { // CCW
            gpio_pin_set_dt(&conf->gpio1, 0);
            gpio_pin_set_dt(&conf->gpio2, 1);
        } else if (power_config.numerator == 0){
            gpio_pin_set_dt(&conf->gpio1, 0);
            gpio_pin_set_dt(&conf->gpio2, 0);
        }
    }

    uint32_t pulse = (conf->pwm.period/power_config.denominator)*power_config.numerator;
